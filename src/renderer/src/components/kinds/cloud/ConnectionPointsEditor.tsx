...
export default function ConnectionPointsEditor({
  ...
  return createPortal(
    <div className="diagen-cpe-overlay" onPointerDown={onCancel}>
      <div
        className="diagen-cpe-modal"
        onPointerDown={(e) => e.stopPropagation()}
        onKeyDown={(e) => e.stopPropagation()}
      >
        <h3 className="diagen-cpe-title">Editar puntos de conexión</h3>
        <p className="diagen-cpe-sub">
          Los 4 puntos centrales siempre están. Añade puntos extra por lado.
        </p>

        <div className="diagen-cpe-body">
          {/* Controles */}
          <div className="diagen-cpe-controls">
            <label className="diagen-cpe-field">
              <span>Lado</span>
            ...
            <div className="diagen-cpe-field">
              <span>Puntos extra</span>
              <div className="diagen-cpe-stepper">
                <button type="button" onClick={() => setCount(count - 1)} disabled={count <= 0}>
                  −
                </button>
                <span className="diagen-cpe-count">{count}</span>
                <button
                  type="button"
          ...
          {/* Preview */}
          <div className="diagen-cpe-preview" style={{ width: PREVIEW, height: PREVIEW }}>
            <div className="diagen-cpe-node" style={{ width: w, height: h }}>
              <img className="diagen-cpe-node-icon" src={getIconDataUri(iconType)} alt="" />
              <span className="diagen-cpe-node-label">{label}</span>
              {/* 4 imanes centrales */}
              {(['top', 'right', 'bottom', 'left'] as Side[]).map((s) => (
                <span key={`c-${s}`} className="diagen-cpe-dot is-center" style={dotStyle(s, 50)} />
              ))}
              {/* Puntos extra */}
              {SIDES.flatMap((s) =>
                extraHandlePositions(extra[s] ?? 0).map((pct, i) => (
                  <span key={`e-${s}-${i}`} className="diagen-cpe-dot" style={dotStyle(s, pct)} />
                ))
              )}
            </div>
          </div>
        ...
        <div className="diagen-cpe-actions">
          <button type="button" className="diagen-btn" onClick={onCancel}>
            Cancelar
          </button>
          <button
            type="button"
            className="diagen-btn diagen-btn-primary"
